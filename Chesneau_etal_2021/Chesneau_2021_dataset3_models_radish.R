...
# abundance file, total number of plants in the experiment
ficname="otu_table_16Sr.csv"
sep=','
Nplanttot=15
...
ASVrad<-read.csv(file = ficname,sep=sep,header=TRUE,)
day=unique(ASVrad$DAP)
...
indexASV=which(substr(colnames(ASVrad),1,3)=="ASV")
C=ASVrad[,indexASV]
...
par(mfrow=c(1,1))
Ntot=rowSums(C)
qu01=quantile(Ntot,probs=c(0.,1))
x=runif(2000, min=qu01[1], max=qu01[2])
#qqplot(x,Ntot)
#qqline(Ntot, distribution = function(p) qunif(p, min=min(Ntot), max=max(Ntot)), probs = c(0., 1), col = 2)
#just to know ...but not so bad on dates separately, except


#loop on dates
for (j in day) {
indexDAP=which(ASVrad$DAP==j)
ASVradj=ASVrad[indexDAP,]
Ntotj=Ntot[indexDAP]
# check if uniform distribution is acceptable (uncomment if necessary)
#qu01=quantile(Ntotj,probs=c(0.,1))
#x=runif(2000, min=qu01[1], max=qu01[2])
#qqplot(x,Ntotj)
#qqline(Ntotj, distribution = function(p) qunif(p, min=min(Ntotj), max=max(Ntotj)), probs = c(0., 1), col = 2)

# and now compute 0.1-0.9 quantiles and store them
qu=quantile(Ntotj,probs=c(0.1,0.9))
lambda=rbind(lambda, qu)
#
#now loop on plants within each day
plant=unique(ASVradj$Plant)
  for (p in plant){
    indexP=which(ASVradj$Plant==p)
    ASVradjp=ASVradj[indexP,]
    nseed=dim(ASVradjp)[1]
# number of seeds per plant and date
    A=transpose(ASVradjp[,indexASV])
    row.names(A)=colnames(ASVradjp[indexASV])
...
#impact of filtering: proportion of non zero data kept after filtering for each plant
hist(Filtering)

# moment estimation of kappa, S and R related to neutral model
popt=NULL
popt2=NULL
popt3=NULL
#
...
wilcox.test(popt[1:10,5],popt[11:15,5],alternative="less")
wilcox.test(popt[5:10,5],popt[11:15,5],alternative="less")
wilcox.test(popt[1:5,5],popt[6:10,5],alternative="greater")



#Write file
write.csv(popt, "popt_r_16S")

#Prepare data plot
poptf <- data.frame(popt)
colnames(poptf) <- c("DAP", "X2", "S", "R", "ratio_Iplant", "X6", "Error")
poptf$DAP<-ifelse(poptf$DAP == 1,'D26',
                  ifelse(poptf$DAP == 2,'D37',
                                ifelse(poptf$DAP == 4,'D65','D65')))
...
#Write file
write.csv(poptf, "poptf_r_16S")

#Open data
poptf_r_16S <- read.csv("poptf_r_16S", sep = ",",  check.names=FALSE, row.names=1)
poptf_r_gyrB <- read.csv("poptf_r_gyrB", sep = ",",  check.names=FALSE, row.names=1)

poptf_r_16S$Plant.species <- c("Radish")
poptf_r_gyrB$Plant.species <- c("Radish")
...
Plot_16Sr_Iplant <- ggplot(data=poptf_r_16S, aes_string(x='DAP',y='ratio_Iplant', fill='DAP')) + 
...
Plot_16Sr_R <- ggplot(data=poptf_r_16S, aes_string(x='DAP',y='R', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_16Sr_Error <- ggplot(data=poptf_r_16S, aes_string(x='DAP',y='Error', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_16Sr_S <- ggplot(data=poptf_r_16S, aes_string(x='DAP',y='S', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_16S_r <- plot_grid(Plot_16Sr_Iplant, Plot_16Sr_R, Plot_16Sr_Error, Plot_16Sr_S)



Plot_gyrBr_Iplant <- ggplot(data=poptf_r_gyrB, aes_string(x='DAP',y='ratio_Iplant', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_gyrBr_R <- ggplot(data=poptf_r_gyrB, aes_string(x='DAP',y='R', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_gyrBr_Error <- ggplot(data=poptf_r_gyrB, aes_string(x='DAP',y='Error', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
Plot_gyrBr_S <- ggplot(data=poptf_r_gyrB, aes_string(x='DAP',y='S', fill='DAP')) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color=DAP)) +
  theme(strip.background = element_rect(color="white", fill="#585858", size=1, linetype="solid"),
        strip.text.x = element_text(size = 12, color = "white"),
...
library(cowplot); packageVersion('cowplot')
Plot_gyrB_r <- plot_grid(Plot_gyrBr_Iplant, Plot_gyrBr_R, Plot_gyrBr_Error, Plot_gyrBr_S)


#Stats
pairwise.wilcox.test(poptf_r_16S[,5], poptf_r_16S[,1], p.adjust.method="BH")
# cool 
pairwise.wilcox.test(poptf_r_16S[,7], poptf_r_16S[,1], p.adjust.method="BH")
# cool

pairwise.wilcox.test(poptf_r_16S[,3], poptf_r_16S[,1], p.adjust.method="BH")
#
wilcox.test(poptf_r_16S[6:10,3],poptf_r_16S[11:15,3],alternative="less")


pairwise.wilcox.test(poptf_r_gyrB[,5], poptf_r_gyrB[,1], p.adjust.method="BH")
# cool 
pairwise.wilcox.test(poptf_r_gyrB[,6], poptf_r_gyrB[,1], p.adjust.method="BH")
# cool
wilcox.test(poptf_r_gyrB[1:10,7],poptf_r_gyrB[11:15,7],alternative="greater")

pairwise.wilcox.test(poptf_r_gyrB[,3], poptf_r_gyrB[,1], p.adjust.method="BH")
#
wilcox.test(poptf_r_gyrB[6:10,3],poptf_r_gyrB[11:15,3],alternative="less")


wilcox.test(poptf_r_gyrB[6:10,5],poptf_r_gyrB[11:15,5],alternative="less")
